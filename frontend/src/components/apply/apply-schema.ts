  ...
  .superRefine((value, ctx) => {
    ...
    if (!value.privacy.dataPrivacyAgreed) {
      issue(
        ...
        "You must agree to the Data Privacy Agreement to continue.",
      );
    }
    ...
    if (committee.applicationType === "position") {
      if (!committee.firstPositionId)
        issue(["committee", "firstPositionId"], "Please select your first choice.");
      if (!committee.secondPositionId)
        issue(["committee", "secondPositionId"], "Please select your second choice.");
      ...
      if (needsPortfolio && !committee.portfolioUrl.trim()) {
        issue(
          ...
          "Add your Google Drive portfolio link for your Creatives committee choice.",
        );
    ...
    if (!committee.motivation.trim())
      issue(["committee", "motivation"], "Please complete all the required fields.");
...
export const applyFormDraftSchema = z.object({
  ...
  general: generalInfoSchema.extend({
    ...
    contactDigits: z.string().default(""),
